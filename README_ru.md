...
DeerFlow выполняет инструменты (bash, файловый ввод/вывод, поиск) непосредственно в процессе хоста. Harness больше не предоставляет изоляции sandbox; для продакшен-развёртывания запускайте сам DeerFlow в контейнере или VM.
...
Каждая задача выполняется в собственном per-thread workspace на хосте с полной файловой системой: skills, workspace, uploads, outputs. Агент читает, пишет и редактирует файлы. Выполняет bash-команды и пишет код. Смотрит на изображения.

DeerFlow выполняет инструменты непосредственно на хосте. Harness больше не предоставляет изоляции sandbox; для продакшен-развёртывания запускайте сам DeerFlow в контейнере или VM.
