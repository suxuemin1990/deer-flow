...
def test_apply_prompt_template_includes_custom_mounts(monkeypatch):
    ...
    monkeypatch.setattr(prompt_module, "_build_acp_section", lambda *_a, **_k: "")
...
def test_apply_prompt_template_includes_relative_path_guidance(monkeypatch):
    ...
    prompt = prompt_module.apply_prompt_template(workspace_path="/run/deerflow/threads/t1/user-data/workspace")

    assert "Treat `/run/deerflow/threads/t1/user-data/workspace` as your default current working directory" in prompt
