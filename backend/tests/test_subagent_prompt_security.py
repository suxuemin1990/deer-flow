...
def test_bash_subagent_prompt_mentions_workspace_relative_paths() -> None:
    ...
    assert "Treat `{workspace_path}` as the default working directory for file IO" in BASH_AGENT_CONFIG.system_prompt
...
def test_general_purpose_subagent_prompt_mentions_workspace_relative_paths() -> None:
    ...
    assert "Treat `{workspace_path}` as the default working directory for coding and file IO" in GENERAL_PURPOSE_CONFIG.system_prompt
